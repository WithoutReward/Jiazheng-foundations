...
public class ServeItemServiceImpl extends ServiceImpl<ServeItemMapper, ServeItem> implements IServeItemService {
    ...
    public void deactivate(Long id) {
        ...
        List<ServeResDTO> serveResDTOS = serveMapper.queryServeListByServeItemId(id);
        for (ServeResDTO serveResDTO : serveResDTOS) {
            if (regionMapper.selectById(serveResDTO.getRegionId()).getActiveStatus() == FoundationStatusEnum.ENABLE.getStatus()) {
                throw new ForbiddenOperationException("有区域在使用该服务项");
            }
        }
